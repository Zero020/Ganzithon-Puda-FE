# 푸다(Fooda) – 음식 기부 예약 플랫폼

<img width="789" height="443" alt="image" src="https://github.com/user-attachments/assets/573c13a4-5424-4f31-99d0-738c2f6e919f" />

<br>

## 목차

1. [프로젝트 소개](#프로젝트-소개)
2. [문제 상황 & 해결 방식](#문제-상황-&-해결-방식)
3. [핵심 기능](#핵심-기능)
4. [기술 스택](#기술-스택)
5. [팀 소개](#팀-소개)
6. [기타 정보](#기타-정보)

<br>

## 프로젝트 소개
푸다는 매일 발생하는 베이커리·카페·식당 등의 남는 음식을
지역 복지시설과 연결하여 낭비는 줄이고, 결식 문제를 완화하는 사회적 가치 기반 서비스입니다.

- 남는 음식 → 기부 가능한 자원으로 전환

- 공공데이터(OpenAPI) 기반 기관 인증

- 영수증 OCR(Upstage API)로 기부의 투명성 & 안전성 확보

- 가게 ↔ 복지시설 실시간 매칭·예약 시스템 제공

<br>

## 문제 상황 & 해결 방식

(1) 남는 음식 연결 부재

- 가게 재고와 시설 수요가 단절되어 동시에 “낭비 + 부족” 발생

(2) 출처 불명·위생 문제

- 개인 중심 공유 플랫폼은 위생성과 출처 검증이 어려움

(3) 기부 흐름 불투명

- 기록 미흡으로 기부 증빙과 수령 관리 불가

#### ✅ 해결책

(1) 실시간 음식 매칭 시스템

- 가게가 등록한 잉여 음식 → 즉시 복지시설과 매칭

(2) 공공데이터 기반 기관 인증

- 사회복지시설정보 OpenAPI로 시설 신뢰도 확보

(3) 기부·수령 자동 기록 시스템

- 예약·픽업·리뷰까지 전 과정을 자동 로그로 기록

<br>

## 핵심 기능

<img width="550" height="310" alt="image" src="https://github.com/user-attachments/assets/037a2d6c-6547-4e80-be6d-ba1d23c720c4" />

<img width="550" height="310" alt="image" src="https://github.com/user-attachments/assets/184a7618-a275-4d88-b151-950400834a6c" />

<img width="550" height="310" alt="image" src="https://github.com/user-attachments/assets/4576d096-6507-44ca-85d7-c003c5d55afd" />

<img width="550" height="310" alt="image" src="https://github.com/user-attachments/assets/d7b56412-9c91-49b9-929f-9e5713df4c50" />

<img width="550" height="310" alt="image" src="https://github.com/user-attachments/assets/eff068f2-f8b4-4773-9f04-02865aa19944" />


<br>

## 기술 스택

<img width="290" height="226" alt="image" src="https://github.com/user-attachments/assets/c74aa845-90c4-467a-ae7a-a029a6198d01" />


<br>

## 팀 소개

<table width="100%" align="center">
  <tr>
    <td align="center">
      <a href="https://github.com/chunbae0523">
        <img src="https://avatars.githubusercontent.com/chunbae0523" width="120" />
      </a><br/>
      김태현<br/>
      팀장, 프론트엔드<br/>
      <small> </small>
    </td>
    <td align="center">
      <a href="https://github.com/Zero020">
        <img src="https://avatars.githubusercontent.com/Zero020" width="120" />
      </a><br/>
      최영<br/>
      디자인, 프론트엔드<br/>
      <small> 랜딩/로그인/회원가입 · 복지시설용</small>
    </td>
    <td align="center">
      <a href="https://github.com/mang3858">
        <img src="https://avatars.githubusercontent.com/mang3858" width="120" />
      </a><br/>
      김소망<br/>
      백엔드<br/>
      <small>OpenAPI 기관 인증 · OCR 기반 리뷰 인증 로직 · 배포 구축</small>
    </td>
    <td align="center">
      <a href="https://github.com/KyungHoon03">
        <img src="https://avatars.githubusercontent.com/KyungHoon03" width="120" />
      </a><br/>
      이경훈<br/>
      백엔드<br/>
      <small> </small>
    </td>
  </tr>
</table>


<br>

## 기타 정보

- 본 프로젝트는 **2025 간지톤 | 멋쟁이사자처럼 12개 대학 연합해커톤**을 위해 제작되었습니다.
- 개발 기간: **2025.10.31 ~ 2025.11.22**
- 기능 개발과 기획, 디자인까지 전 팀원이 협업하여 완성한 프로젝트입니다.
- 본 리포지토리는 '푸다' 서비스의 프론트엔드 리포지토리입니다.
#### 📎 백엔드 리포지토리: https://github.com/Ganzithon-team6/team6-BE

